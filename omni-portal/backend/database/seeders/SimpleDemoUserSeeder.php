...
class SimpleDemoUserSeeder extends Seeder
{
    ...
    public function run(): void
    {
        DB::transaction(function () {
            ...
            if (User::where('email', 'demo@example.com')->exists() && User::where('email', 'demo@test.com')->exists()) {
                $this->command->info("Demo users already exist!");
                return;
            }
            ...
            if (!User::where('email', 'demo@example.com')->exists()) {
                ...
                // Create beneficiary profile (existing code)
            ...
            // Create test user with your credentials
            if (!User::where('email', 'demo@test.com')->exists()) {
                ...
                // Create beneficiary profile for test user
                $testBeneficiary = Beneficiary::create([
                    'user_id' => $testUser->id,
                    'cpf' => $testUser->cpf,
                ...
                // Create gamification progress for test user
                GamificationProgress::create([
            ...
            $this->command->info("🏆 Level: 1 with 100 points");
            
            $this->command->info("\n🚀 You can now login with either set of credentials!");
        });
    }
}